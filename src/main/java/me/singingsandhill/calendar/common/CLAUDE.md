...
**진입점 분리:** 어드민 경로(`/runners/admin/**`, `/trading`, `/trading/**`, `/api/trading/**`,
`/api/stock/bot/**`) 5개를 `defaultAuthenticationEntryPointFor` 로 먼저 매핑하고, 그 외 전체는 마지막
`AnyRequestMatcher.INSTANCE` 매핑(카카오 `/login`)으로 처리한다.
