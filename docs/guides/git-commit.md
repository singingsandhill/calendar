...
# Commit 97 — feat(stock): 유니버스 거래량순위 스크리닝 시점 재시도 + cttr/rank 계측 + 청산시각 드리프트 정리 (ADR 0006)
...
# Commit 98 — feat(stock): 스크리닝 메일 첨부 스냅샷 명시 + logback 크립토/주식 로그 분리 — 마지막 커밋(git-commit.md 포함)
...
# Commit 100 — fix(stock): 상태머신 영속화 복원 + TP1 수량 캡 + 주문 무재시도 (P0-3/5/4)
...
# Commit 101 — feat(stock): PAPER 기본 모드 — LIVE 는 STOCK_BOT_MODE=LIVE opt-in (P0-2, ADR stock/modes/0002)
...
# Commit 102 — feat(common): 주식 봇 제어 API 관리자 전용 (P0-1, ADR common/security/0005) — 마지막 커밋(git_commit.md 포함)
...
# Commit 103 — fix(stock): 청산 구조 재보정 — 풀백저가 손절 + 진입가 앵커 TP3 + 트레일링 정상화 (ADR stock/algorithm/0007)
...
# Commit 104 — feat(stock): 주문 선영속화 + 실체결 backfill + 고아 체결 스윕 (ADR stock/infrastructure/0006)
...
# Commit 105 — feat(stock): 재시작 보호 전용 자동 재개 + 최종청산 재시도·알림 (ADR stock/modes/0003) — 마지막 커밋(git_commit.md 포함)
...
# Commit 106 — fix(stock): 매도세율 2026 기준 0.20% 정정 + 익절 게이트에 슬리피지 반영 (P2-1)
...
# Commit 107 — feat(stock): 강제 선정 제거 + 신호 팩터 게이트 + 거래정지·VI 가드 (P2-2/3, ADR stock/algorithm/0008)
...
# Commit 108 — feat(stock): 일일 실적 요약 리포트 + 문서 드리프트 해소 (P2-5/4) — 마지막 커밋(git_commit.md 포함)
...
# Commit 109 — chore(stock): PAPER 모드 체결 조회 스킵 + BACKTEST 서술 정정 (문서-코드 드리프트)
...
# Commit 110 — fix(stock): 진입 풀백 하한 1.5% + 트레일링 2.0% — 비용 게이트 정합 (ADR stock/algorithm/0009)
...
# Commit 111 — fix(stock): 진입검증 시간조건 null=FAIL + 매도 원장 비용 기록 + tp2-ratio 배선
...
# Commit 112 — feat(trading): 기본 모드 PAPER 전환 + Java 기본값 yaml 정합 (ADR trading/modes/0002, P1-8/P1-9)
git add -p src/main/resources/application.yaml
...
# Commit 113 — test(trading): 손절/익절/트레일링 트리거 경계 테스트 (P1-5) — 마지막 커밋(git_commit.md 포함)
...
# Commit 114 — docs: CLAUDE.md 전수 감사 — 사실 오류 6건 정정 + 누락 동작·열거 보강
