...
**보호 엔드포인트** (`SecurityConfig` 는 선언 순서가 곧 매칭 우선순위 — 각 보호 규칙은 자신을
삼킬 수 있는 포괄 permitAll(`/api/**`, `/api/stock/**`, `/*`)보다 **먼저** 선언되어 있다):

| 경로 패턴 | 필요 권한 |
|-----------|----------|
| `/api/trading/**`, `/trading`, `/trading/**` | `ROLE_ADMIN` (봇 제어·실주문·제어 대시보드) |
| `POST /api/stock/bot/**` | `ROLE_ADMIN` (`GET .../status` 는 공개 대시보드 위젯용) |
| `/me`, `/recap`, `/recap/**`, `/api/me/**` | `ROLE_USER` (카카오 로그인) |
| `/runners/admin`, `/runners/admin/**` | `ROLE_ADMIN` |
| 그 외 | `authenticated` |
...
| 경로 패턴 | 대상 |
|-----------|------|
| `/recap/share/**` | 리캡 공유 링크 — 반드시 `/recap/**` 보호 규칙보다 **먼저** 선언 |
| `/login`, `/oauth2/**`, `/login/oauth2/**` | 카카오 로그인 |
| `/`, `/start`, `/index.html`, `/privacy-policy`, `/about` | 홈페이지 |
| `/api/**` | REST API (위 보호 규칙에 걸린 경로는 제외) |
| `/css/**`, `/js/**`, `/images/**` 등 | 정적 리소스 |
| `/h2-console/**` | H2 콘솔 (개발용) |
| `/runners`, `/runners/announce`, `/runners/runs/**`, `/runners/members/**`, `/runners/{css,js,images}/**`, `/runners/admin/login` | 러너 공개 페이지 — `/runners/**` 와일드카드가 **아니다**(`/runners/admin/**` 를 삼키지 않도록 개별 매처로 나열) |
| `/insights(/**)`, `/use-cases(/**)`, `/tools(/**)` | 인사이트·유스케이스·도구 |
| `/stock`, `/stock/**`, `/api/stock/**` | 주식봇 대시보드·조회 API |
| `/*`, `/*/*/*` | 동적 경로 (owner 페이지 등) — `/**` 전역 와일드카드는 없다 |

> ⚠️ 보호 규칙을 포괄 permitAll(`/api/**`, `/*`) 뒤로 옮기면 **트레이딩 실주문 API 와 카카오
> 사용자 영역이 무인증 공개된다.** `/runners/admin/login`(공개)도 `/runners/admin/**`(ADMIN)보다
> 먼저 와야 로그인 페이지 자체가 ADMIN 을 요구하지 않는다.

### 인증 흐름

- **러너 관리자**: 폼 기반 로그인 (`/runners/admin/login`, 로그아웃 후 `/runners`)
- **카카오 사용자**: OAuth2 로그인 (`/login` → `KakaoOAuth2UserService`, 성공 시 `/me`)
- **비밀번호 인코딩**: BCrypt
...
| 스케줄 | 크론 표현식 | 메서드 | 설명 |
|--------|------------|--------|------|
| 매매 루프 | `5 * * * * *` (매분 5초) | `executeTradeLoop()` | 미결 주문 스윕 → 캔들 갱신 → 리스크 체크 → 시그널 생성 → 리밸런싱/매매 |
| 캔들 동기화 | `0 */5 * * * *` (5분 간격) | `syncCandles()` | 누락 캔들 보정 |
| 오래된 캔들 정리 | `0 0 0 * * *` (매일 자정) | `cleanupOldCandles()` | `trading.bot.candle-retention-days`(기본 90일) 이전 캔들 삭제 — `bot.enabled` 가드 없이 항상 실행 |
...
| 스케줄 | 크론/설정 | 메서드 | 설명 |
|--------|----------|--------|------|
| 프리마켓 | `0 30 8 * * MON-FRI` (08:30) | `executePreMarket()` | 당일 유니버스 정적 스냅샷 (거래량순위 미호출) |
| 스크리닝 | `0 20 9 * * MON-FRI` (09:20) | `executeScreening()` | 갭 종목 선별 + 결과 메일 발송 |
| 매매 루프 | `fixedRate: 5초` | `executeTradingLoop()` | 09:20~11:20 상태 업데이트 + 진입/청산 |
| 최종 청산 | `0 20 11 * * MON-FRI` (11:20) | `executeFinalExit()` | 보유 포지션 전량 청산 (3회 재시도 + 실패 시 메일) |
| 일일 리포트 | `0 40 11 * * MON-FRI` (11:40) | `executeDailyReport()` | 당일 실적 요약 메일 |

### Stock 일일 매매 타임라인

```
08:30          09:00        09:20                        11:20        11:40
  │              │            │                            │            │
  ▼              ▼            ▼                            ▼            ▼
프리마켓      장 시작    스크리닝 + 매매 루프 시작       최종 청산    일일 리포트
(유니버스                (갭 종목 선별 / 5초 간격          (전량        (승률·손익
 정적 스냅샷)             상태 전이 + 진입/청산)            매도)        요약 메일)
```
...
```
매 1분마다 실행 (cron 5 * * * * *):
  0. 미결(SUBMITTED) 주문 스윕
  1. 캔들 갱신 (BithumbPublicApi → CandleService)
     — 실패해도 리스크 체크는 계속하고 신규 매매만 억제 (ADR trading/risk/0006)
  2. 리스크 체크 (SL -1.5%, TP +3%, 트레일링) ← 항상 최우선 (순서 불변식)
  3. 시그널 생성 (지표 MA5/20/60·RSI14·Stochastic%K/%D → 점수 기반 BUY/SELL/HOLD)
  4. 강한 신호(|점수| ≥ 60)면 리밸런싱보다 먼저 매매
  5. 아니면 리밸런싱 → 미실행 시 신호대로 매매 실행 (BithumbPrivateApi)
```

### 시그널 스코어링 시스템

시그널은 여러 지표의 점수를 합산하여 결정한다 (합산 범위 ±128점).

| 지표 | 점수 범위 | 기준 |
|------|----------|------|
| MA 크로스 | ±25 (크로스 이벤트) / ±5 (MA5-MA20 상태) | 골든/데스 크로스, MA 수렴(<0.2%) 시 0 |
| MA 추세 | ±8 | 가격 vs MA60 위치 |
...
**매매 결정**:
- **매수**: 점수 ≥ 40 AND RSI < 70 AND StochK < 85 AND 동의 지표 ≥ 3 (현재가 < MA60 이면 강세 다이버전스·RSI < 30·거래량 스파이크 중 하나 필수)
- **매도**: 점수 ≤ -40 AND RSI > 30 AND StochK > 15 AND 동의 지표 ≥ 3
- **홀드**: 그 외
...
| 규칙 | 조건 | 동작 |
|------|------|------|
| 손절 (Stop-Loss) | 수수료 반영 손익률 ≤ -1.5% | 전량 매도 |
| 익절 (Take-Profit) | 수수료 반영 손익률 ≥ +3% | 전량 매도 |
| 트레일링 스탑 | 최고점 대비 -0.8% 하락 (진입가 + 왕복 수수료 하한) | 전량 매도 |
| 트레일링 활성화 | 수익률 +1.5% 도달 시 활성화 | 추적 시작 |

- 수수료율: 0.25%
- 슬리피지 버퍼: 0.5%
- 최소 수익 임계값: 0.1% (`trading.risk.min-profit-threshold`, 순수 net 마진 — 이하일 때 매도 스킵)
...
| 상태 | 설명 | 전이 조건 |
|------|------|----------|
| `WATCHING` | 갭 스크리닝 통과, 감시 시작 | 갭 0.5~15%, 체결강도 ≥ 95, 시총 ≥ 500억, 총점 ≥ 40 |
| `HIGH_FORMED` | 고점 형성 확인 | 가격 ≥ 시가 × 1.015 |
| `PULLBACK` | 눌림목 발생 | 고점 대비 1.5~5.0% 하락 |
| `ENTRY_READY` | 반등 확인, 진입 대기 | 눌림목 저점 대비 +0.2% 반등 + 진입 검증 2/3 (체결강도 ≥ 100, 호가 불균형 ≥ 1.0, 풀백 지속 3~15분) |
| `ENTERED` | 포지션 보유 중 | 매수 주문 체결 |
| `EXITED` | 포지션 청산 완료 | 익절/손절/시간청산 |
| `FILTERED_OUT` | 감시 제외 | 고점 대비 5.0% 초과 하락 (`entry.pullback-max-percent`) |
...
| 단계 | 조건 | 매도 비율 | 설명 |
|------|------|----------|------|
| TP1 | 가격 ≥ 진입가 × 1.05 (+5%, `exit.tp1-percent`) | 진입수량의 50% (`tp1-ratio`, 잔여 캡) | 부분 익절 |
| TP2 | 가격 ≥ 당일 고점 | 잔량의 60% (`tp2-ratio`) | 추가 수익 확보 |
| TP3 | 가격 ≥ 진입가 × 1.10 (+10%, `exit.tp3-percent`) | 잔량 전부 | 앵커는 진입가 고정 |
| 트레일링 | 부분익절 후 활성 — 가격 ≤ 트레일링 고점 × 0.98 (`risk.trailing-stop-percent` 2.0%) | 잔량 전부 | 손익분기 하한 유지 |
| 손절 | 가격 ≤ max(풀백저가 × 0.99, 진입가 × 0.98) (`pullback-stop-buffer-percent` 1.0% / `max-stop-loss-percent` 2.0%) | 전량 | 손실 제한 |
| 시간 청산 | 11:20 KST | 전량 | 당일 청산 원칙 |

### 스크리닝 필터

운영값은 `stock.scoring.enabled: true` (점수 기반 모드). 아래는 그 경로의 실효 기준이다 —
`screening.min-gap-percent`·`max-gap-percent`·`min-market-cap` 등은 legacy 경로 전용 키다.

| 단계 | 필터 | 기준 |
|------|------|------|
| Floor 0 | 거래 가능 상태 | VI·거래정지 등 배제 |
| Floor 1 | 갭 비율 | `screening.floor-gap-percent` 0.5% ~ `scoring.floor-max-gap` 15% |
| Floor 2 | 체결강도 | ≥ `screening.floor-trade-strength` 95 (미집계(0)는 `skip-zero-strength` 기본 true 로 탈락) |
| Floor 3 | 시가총액 | ≥ `scoring.floor-min-market-cap` 500억 원 |
| 점수 | 가중 합산 | 갭 30 / 체결강도 25 / 거래대금 20 / 스프레드 15 / 시총 10 |
| 게이트 | 총점 + 신호 팩터 | 총점 ≥ `min-score-threshold` 40 **AND** 갭+체결강도 ≥ `signal-min-score` 25 |
| 최종 | 감시 목록 | 최대 `max-watchlist-size` 10개 (점수 미달 강제 선정 없음) |
