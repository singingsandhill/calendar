...
- **긍정:**
  - PAPER 모드로 실주문 없이 알고리즘 검증. 단 **시세·호가·잔고 조회는 모드 무관 실 API** 라
    자금이 아주 없어도 되는 건 아니다 — 포지션 사이징이 실계좌 예수금·매수가능수량을 참조하므로
    예수금 0 이면 가상 진입도 발생하지 않는다 (2026-07-24 리뷰 후속 확인).
  - BACKTEST 모드로 과거 데이터 시뮬레이션 (별도 데이터 fixture 필요).
    **미구현 — 2026-07-24 기준 BACKTEST 는 PAPER 와 동일하게 동작한다**(모드 분기가
    `isLiveMode()` 하나뿐이라 시세는 실 API). 실측에는 PAPER 를 사용할 것.
