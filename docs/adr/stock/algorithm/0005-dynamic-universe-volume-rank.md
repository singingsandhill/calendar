...
- **부정 / 주의:**
  - **라이브 검증 완료(부분)**: TR `FHPST01710000` 호출과 단축코드 파싱(`mksc_shrn_iscd`,
    부재 시 `stck_shrn_iscd` 폴백)은 2026-08-03 운영 로그에서 실계좌 응답으로 확인됐다 —
    `Volume-rank returned 1 codes` 후 그 코드가 시세·체결강도·호가 조회를 거쳐 스코어링까지
    진행됐다. 남은 미확인은 **FID 파라미터**다: 30건 요청에 1건만 돌아왔고, ETF/ETN 이 섞여
    드는 `FID_TRGT_EXLS_CLS_CODE`(현재 `"0000000000"`) 비트마스크는 공식 스펙 미확인이라
    [0010](0010-universe-degradation-threshold.md) 의 후속 항목으로 남아 있다.
