...
- **운영 실측 (2026-09-19):** "수 초"로 추정했던 요청 스레드 점유가 실제로는 Hikari 커넥션
  점유 시간과 같았다 — `analyze()` 가 클래스 레벨 `@Transactional(readOnly=true)` 로 묶여 있어
  90일 집계(13만 행 Java 연산) 내내 커넥션 1개를 붙잡았고, 운영 서버에서 스케줄러(1분 봉
  루프·주식 5초 루프)와 같은 기본 Hikari 풀(10)을 공유하는 상태에서 사이트 전역 504 로
  번졌다. 조치: 읽기 3건만 `TransactionTemplate`(주문 경로와 동일 패턴, P0-3b)로 짧게 묶고
  집계는 트랜잭션 밖에서 실행 — 이 ADR 의 결정(온-리드 집계·투영·온디맨드) 자체는 바뀌지
  않았다. 페이지가 요청 스레드를 수 초 잡는 것은 여전히 유효한 트레이드오프다.
