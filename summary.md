...
| 결정의 *왜* (HTTPS 통일, sitemap lastmod, UniverseBuilder, TP 비순차화 등 59개 결정) | [`docs/adr/README.md`](docs/adr/README.md) |
...
- 코인 봇 점수 합산 범위: **±128** = 25+8+20+15+15+15+20+10 (`SignalService`, 모멘텀
  가중 하향 후 — 원 ±135).
- Trading 리스크 기본값 (`application.yaml`): stop-loss `-1.5%`, take-profit `+3%`,
  trailing-activation `+1.5%`, trailing-stop `-0.8%`, takerFee `0.25%`, minProfit `0.1%`.
