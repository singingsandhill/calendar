...
- **문서를 사실의 근거로 인용하지 않는다(이 파일 포함).** 심볼·경로·수치는 코드에서 확인한다 —
  문서가 존재하지 않는 심볼을 지목한 전례가 있다 (`docs/git_commit.md` Commit 114).
...
- **커밋·푸시를 직접 실행하지 않는다.** 변경 후 `docs/git_commit.md` 에 규약에 맞는 섹션을 덧붙인다.
- 형식 규약은 `docs/guides/git-commit.md` 헤더 참고 — cmd.exe 호환(큰따옴표만), `git add` 한 줄,
  같은 파일이 여러 커밋에 걸리면 처음 등장하는 커밋이 소유, 배치의 마지막 커밋이 로그 파일 자신을 포함.
